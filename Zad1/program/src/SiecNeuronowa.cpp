...
void SiecNeuronowa::dodajWarstwe(int lb_neur, int lb_wejsc, bool akt) {
    Warstwa *warstwa = new Warstwa;
    warstwa->stworzNeurony(lb_neur, lb_wejsc, akt);
...
void SiecNeuronowa::firstB(double *odp) {
    ...
    for (int i = 0; i < warstwy.back()->getNeurony().size(); i++) {
        wynik = (warstwy.back()->getY()[i] - odp[i]) *
                warstwy.back()->getNeurony()[i]->pochodnaAktywacji(warstwy.back()->getWejscia()); //(y-odp) * f'(suma)
        warstwy.back()->getNeurony()[i]->setB(
                wynik); //zapisujemy ta wartosc dla kazdego neurony z warstwy wyjsciowej w danym wzorcu
    }
}
